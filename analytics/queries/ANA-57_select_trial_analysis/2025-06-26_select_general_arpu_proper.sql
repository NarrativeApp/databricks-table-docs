-- ANA-57: General Select Product ARPU Analysis (Based on example pattern)
-- Calculates ARPU for all Select customers using invoice line items approach

-- 1️⃣ Actual Select plan nicknames (paid plans only)
WITH paid_plan_names(nickname) AS (
    VALUES
...
select_charges AS (
    ...
    WHERE i.amount_paid > 0
        ...
        AND i.date >= '2023-01-01'  -- Last 30 months for broader history
        AND i.paid = true
        AND i.billing_reason = 'subscription_cycle'  -- Regular billing
),

-- 3️⃣ Among those Select charges, grab the new-nickname ones and add plan details
new_plan_charges AS (
    SELECT 
        sc.*,
    ...
    WHERE p.nickname IN (SELECT nickname FROM paid_plan_names)
),
...
-- 5️⃣ Calculate ARPU metrics by month
...
-- 6️⃣ Final results with growth metrics
SELECT 
    revenue_month,
    paying_customers,
