-- ANA-27: Sign up completion rates over time - Google vs Email (Rolling 7-day windows)
-- Shows more granular daily changes in completion rates using rolling 7-day periods
-- Date: 2025-06-25
...
-- Create 7-day rolling windows manually since we need more granular data
rolling_7d_windows AS (
    SELECT DISTINCT
        d.start_date - INTERVAL 6 DAY as window_start,
        d.start_date as window_end
    FROM (
        SELECT DISTINCT start_date 
        FROM signup_funnel 
        WHERE start_date >= '2023-04-01'
    ) d
    WHERE d.start_date >= '2023-04-07'  -- Need 7 days of history
        AND d.start_date <= CURRENT_DATE - INTERVAL 1 DAY
),

rolling_windows AS (
    -- Calculate metrics for each 7-day window
    SELECT 
        w.window_start,
        w.window_end,
        f.sign_up_method,
    ...
    FROM rolling_7d_windows w
    LEFT JOIN signup_funnel f 
        ON f.start_date >= w.window_start 
        AND f.start_date <= w.window_end
...
-- Calculate completion rates for each 7-day window - FULL GRANULAR TREND
SELECT 
    window_start,
    window_end,
    sign_up_method,
...
WHERE sign_up_method IS NOT NULL  -- Filter out windows with no sign-up data
    AND total_starts >= 5  -- Only include windows with meaningful sample size (lower threshold for 7-day)
ORDER BY window_start ASC, sign_up_method  -- Chronological order to see full trend